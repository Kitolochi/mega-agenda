...
import { playTick, playCelebrate, playError } from '../utils/sounds'
...
export default function TaskItem({ task, compact, onToggle, onDelete }: TaskItemProps) {
  ...
  if (compact) {
    return (
      <div
        ...
        } ${animating ? 'task-celebrate' : ''} ${
          !task.completed && task.priority === 1 ? 'bg-accent-red/[0.06]' : 'hover:bg-white/[0.06]'
        }`}
      >
        <button
          ...
          className={`w-3.5 h-3.5 rounded-[4px] border-[1.5px] flex-shrink-0 flex items-center justify-center transition-all duration-200 ${
            task.completed ? 'bg-accent-emerald border-accent-emerald shadow-sm shadow-accent-emerald/30' : priorityColors[task.priority as 1|2|3]
        ...
        <span className={`text-[11px] truncate flex-1 transition-all duration-300 ${task.completed ? 'line-through text-muted' : 'text-white/80'}`}>
          {task.title}
        </span>
        ...
        {task.due_date && !task.completed && (
          <span className={`text-[10px] ${isOverdue ? 'text-accent-red animate-pulse-soft' : 'text-muted'}`}>
  ...
  return (
    <div
      className={`glass-card flex items-start gap-3 p-3 rounded-xl transition-all duration-300 group ${
        task.completed ? 'opacity-50' : task.priority === 1 ? 'border-accent-red/20' : ''
      } ${animating ? 'task-celebrate' : 'hover:translate-y-[-1px]'}`}
    ...
    >
      <button
        ...
        className={`w-[18px] h-[18px] rounded-[5px] border-[1.5px] flex-shrink-0 flex items-center justify-center mt-0.5 transition-all duration-200 ${
          task.completed ? 'bg-accent-emerald border-accent-emerald shadow-sm shadow-accent-emerald/30' : priorityColors[task.priority as 1|2|3]
        } ${animating ? 'scale-130 animate-glow-ring' : 'hover:scale-110'}`}
      >
        {task.completed && (
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-[13px] leading-snug transition-all duration-300 ${task.completed ? 'line-through text-muted' : 'text-white/90'}`}>
          {task.title}
        </p>
        {task.description && (
        ...
        <div className="flex items-center gap-1.5 mt-1.5">
          {!task.completed && (
            <span className={`text-[10px] px-1.5 py-[1px] rounded-md font-medium transition-all duration-200 ${
          ...
          {task.due_date && (
            <span className={`text-[10px] ${isOverdue ? 'text-accent-red font-medium animate-pulse-soft' : 'text-muted'}`}>
      ...
      {onDelete && (
        <button
          onClick={() => { onDelete(); playError() }}
          className="text-muted/40 hover:text-accent-red p-1 rounded-lg hover:bg-accent-red/10 transition-all duration-200 opacity-0 group-hover:opacity-100 hover:scale-110 active:scale-95"
        >
