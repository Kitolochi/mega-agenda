...
export default function MorningBriefing() {
  ...
  if (loading) {
    return (
      <div className="glass-card-elevated rounded-xl overflow-hidden animate-fade-in">
  ...
  return (
    <div className="glass-card-elevated rounded-xl overflow-hidden animate-scale-in">
      <div className="h-[3px] gradient-bar" />
      <div className="p-4">
        <div className="flex items-center justify-between mb-2.5 cursor-pointer press-effect" onClick={() => setCollapsed(!collapsed)}>
          <div className="flex items-center gap-2">
            <span className="text-sm animate-float">✨</span>
            <span className="text-[10px] uppercase tracking-widest font-display font-medium gradient-text">Daily Briefing</span>
            {briefing.isAiEnhanced && (
              <span className="text-[8px] px-1.5 py-0.5 rounded-full bg-accent-purple/15 text-accent-purple font-medium animate-pulse-soft">AI</span>
            )}
          </div>
          <button
            className="text-muted hover:text-white/60 transition-all duration-200"
        ...
        {!collapsed && (
          <div className="text-xs text-white/70 leading-relaxed whitespace-pre-line animate-fade-in">
            {briefing.content}
          </div>
        )}
      </div>
